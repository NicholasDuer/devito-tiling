...
def plot_elapsed_times_bars():
    for so in space_orders:
        ...
        results.plot(x='Dimensions', kind='bar', rot=10, ylabel="Time elapsed (s)",title="Elapsed Times, HPC Laplace Experiments, SO: " + str(so))
        plt.savefig(graphs_folder + "elapsed_time_" + str(so) + "so")

        results = get_result_df('computation_time')
        results.plot(x='Dimensions', kind='bar', rot=10, ylabel="Computation time (s)",title="Computation Times, HPC Laplace Experiments, SO: " + str(so))
