...
def generate_graph(space_order):
    ...
    for standard_row in standard_csv:
        ...
        results.append([experiment_name, get_avg_time(standard_times), get_avg_time(overlapped_times), get_avg_time(openmp_times)])
    
    def save_graph(results):
        df = pd.DataFrame(results, columns=['Dimensions', 'Standard MPI', 'Overlapped Tiling MPI', 'OpenMP'])
