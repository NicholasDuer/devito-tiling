        ...
        for (int y0_blk0 = y_m - lower_y_offset; y0_blk0 <= y_M + upper_y_offset; y0_blk0 += y0_blk0_size)
        {
          for (int x = x0_blk0; x <= MIN(x0_blk0 + x0_blk0_size - 1, x_M + upper_x_offset); x += 1)
          {
            for (int y = y0_blk0; y <= MIN(y0_blk0 + y0_blk0_size - 1, y_M + upper_y_offset); y += 1)
            {
              ...
              {
              float r4 = -1.42361111100763F*u[t0][x + 16][y + 16][z + 16];
              u[t1][x + 16][y + 16][z + 16] = dt*(r0*(r4 - 8.9285714284415e-4F*(u[t0][x + 12][y + 16][z + 16] + u[t0][x + 20][y + 16][z + 16]) + 1.26984126982279e-2F*(u[t0][x + 13][y + 16][z + 16] + u[t0][x + 19][y + 16][z + 16]) - 1.00000000005821e-1F*(u[t0][x + 14][y + 16][z + 16] + u[t0][x + 18][y + 16][z + 16]) + 8.00000000046566e-1F*(u[t0][x + 15][y + 16][z + 16] + u[t0][x + 17][y + 16][z + 16])) + r1*(r4 - 8.9285714284415e-4F*(u[t0][x + 16][y + 12][z + 16] + u[t0][x + 16][y + 20][z + 16]) + 1.26984126982279e-2F*(u[t0][x + 16][y + 13][z + 16] + u[t0][x + 16][y + 19][z + 16]) - 1.00000000005821e-1F*(u[t0][x + 16][y + 14][z + 16] + u[t0][x + 16][y + 18][z + 16]) + 8.00000000046566e-1F*(u[t0][x + 16][y + 15][z + 16] + u[t0][x + 16][y + 17][z + 16])) + r2*(r4 - 8.9285714284415e-4F*(u[t0][x + 16][y + 16][z + 12] + u[t0][x + 16][y + 16][z + 20]) + 1.26984126982279e-2F*(u[t0][x + 16][y + 16][z + 13] + u[t0][x + 16][y + 16][z + 19]) - 1.00000000005821e-1F*(u[t0][x + 16][y + 16][z + 14] + u[t0][x + 16][y + 16][z + 18]) + 8.00000000046566e-1F*(u[t0][x + 16][y + 16][z + 15] + u[t0][x + 16][y + 16][z + 17])) + r3*u[t0][x + 16][y + 16][z + 16] + 1.0e-1F);
             }
            }
          }
        }
...
static void haloupdate0(struct dataobj *restrict u_vec, MPI_Comm comm, struct neighborhood * nb, int otime)
{
  sendrecv0(u_vec,u_vec->hsize[3],u_vec->npsize[2],u_vec->npsize[3],otime,u_vec->oofs[2],u_vec->hofs[4],u_vec->hofs[6],otime,u_vec->hofs[3],u_vec->hofs[4],u_vec->hofs[6],nb->rcc,nb->lcc,comm);
  sendrecv0(u_vec,u_vec->hsize[2],u_vec->npsize[2],u_vec->npsize[3],otime,u_vec->oofs[3],u_vec->hofs[4],u_vec->hofs[6],otime,u_vec->hofs[2],u_vec->hofs[4],u_vec->hofs[6],nb->lcc,nb->rcc,comm);
  sendrecv0(u_vec,u_vec->npsize[1],u_vec->hsize[5],u_vec->npsize[3],otime,u_vec->hofs[2],u_vec->oofs[4],u_vec->hofs[6],otime,u_vec->hofs[2],u_vec->hofs[5],u_vec->hofs[6],nb->crc,nb->clc,comm);
