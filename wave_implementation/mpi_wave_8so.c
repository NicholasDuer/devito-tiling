  ...
  for (int time_tile_base = time_m; time_tile_base <= time_M; time_tile_base += time_tile_size)
  {
    ...
    for (int wf_time_base = time_tile_base; wf_time_base <= time_tile_base + time_tile_size - 1; wf_time_base += wf_height)
    {
      ...
      for (int wf_x_base = x_m - outer_offset; wf_x_base <= x_M + outer_offset; wf_x_base += wf_x_width) 
      {
        ...
        {
          int wf_offset = 0;
          for (int time = wf_time_base, t0 = (time)%(3), t1 = (time + 2)%(3), t2 = (time + 1)%(3); time <= MIN(MIN(time_M, wf_time_base + wf_height - 1), time_tile_base + time_tile_size - 1); time += 1, t0 = (time)%(3), t1 = (time + 2)%(3), t2 = (time + 1)%(3))
          {
            int outer_offset_single_layer = ((time_tile_size - 1) - (time % time_tile_size)) * angle;
            int lower_x_offset = outer_offset_single_layer;
            int upper_x_offset = outer_offset_single_layer; 
            int lower_y_offset = outer_offset_single_layer;
            int upper_y_offset = outer_offset_single_layer;
            if (isleft) {
              lower_x_offset = 0;
            }
            if (isright) {
            ...
            #pragma omp parallel num_threads(nthreads)
            {
              ...
              for (int x0_blk0 = MAX(wf_x_base - wf_offset, x_m - lower_x_offset); x0_blk0 <= wf_x_base + wf_x_width - wf_offset - 1; x0_blk0 += x0_blk0_size) 
              {
                for (int y0_blk0 = MAX(wf_y_base - wf_offset, y_m - lower_y_offset); y0_blk0 <= wf_y_base + wf_y_width - wf_offset - 1; y0_blk0 += y0_blk0_size) 
                {
                  ...
                  {
                    for (int y = y0_blk0; y <= MIN(MIN(y_M + upper_y_offset, wf_y_base + wf_y_width - wf_offset - 1), y0_blk0 + y0_blk0_size - 1); y += 1)
                    {
                      #pragma omp simd aligned(damp,u,vp:32)
                      for (int z = z_m; z <= z_M; z += 1)
                      {
                        float r2 = 1.0F/(vp[x + kernel_offset][y + kernel_offset][z + kernel_offset]*vp[x + kernel_offset][y + kernel_offset][z + kernel_offset]);
                        u[t2][x + kernel_offset][y + kernel_offset][z + kernel_offset] = (r1*damp[x + kernel_offset][y + kernel_offset][z + kernel_offset]*u[t0][x + kernel_offset][y + kernel_offset][z + kernel_offset] + r2*(-r0*(-2.0F*u[t0][x + kernel_offset][y + kernel_offset][z + kernel_offset]) - r0*u[t1][x + kernel_offset][y + kernel_offset][z + kernel_offset]) + 1.78571425e-5F*(-u[t0][x + kernel_offset - 4][y + kernel_offset][z + kernel_offset] - u[t0][x + kernel_offset][y + kernel_offset - 4][z + kernel_offset] - u[t0][x + kernel_offset][y + kernel_offset][z + kernel_offset - 4] - u[t0][x + kernel_offset][y + kernel_offset][z + kernel_offset + 4] - u[t0][x + kernel_offset][y + kernel_offset + 4][z + kernel_offset] - u[t0][x + kernel_offset + 4][y + kernel_offset][z + kernel_offset]) + 2.53968248e-4F*(u[t0][x + kernel_offset - 3][y + kernel_offset][z + kernel_offset] + u[t0][x + kernel_offset][y + kernel_offset - 3][z + kernel_offset] + u[t0][x + kernel_offset][y + kernel_offset][z + kernel_offset - 3] + u[t0][x + kernel_offset][y + kernel_offset][z + kernel_offset + 3] + u[t0][x + kernel_offset][y + kernel_offset + 3][z + kernel_offset] + u[t0][x + kernel_offset + 3][y + kernel_offset][z + kernel_offset]) + 1.99999996e-3F*(-u[t0][x + kernel_offset - 2][y + kernel_offset][z + kernel_offset] - u[t0][x + kernel_offset][y + kernel_offset - 2][z + kernel_offset] - u[t0][x + kernel_offset][y + kernel_offset][z + kernel_offset - 2] - u[t0][x + kernel_offset][y + kernel_offset][z + kernel_offset + 2] - u[t0][x + kernel_offset][y + kernel_offset + 2][z + kernel_offset] - u[t0][x + kernel_offset + 2][y + kernel_offset][z + kernel_offset]) + 1.59999996e-2F*(u[t0][x + kernel_offset - 1][y + kernel_offset][z + kernel_offset] + u[t0][x + kernel_offset][y + kernel_offset - 1][z + kernel_offset] + u[t0][x + kernel_offset][y + kernel_offset][z + kernel_offset - 1] + u[t0][x + kernel_offset][y + kernel_offset][z + kernel_offset + 1] + u[t0][x + kernel_offset][y + kernel_offset + 1][z + kernel_offset] + u[t0][x + kernel_offset + 1][y + kernel_offset][z + kernel_offset]) - 8.54166647e-2F*u[t0][x + kernel_offset][y + kernel_offset][z + kernel_offset])/(r0*r2 + r1*damp[x + kernel_offset][y + kernel_offset][z + kernel_offset]);
                      }
                    } 
                  }
                }
